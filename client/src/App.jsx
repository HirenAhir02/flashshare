...
const Navbar = ({ darkMode, toggleTheme, setPage }) => (
  <nav className={`fixed top-0 w-full z-50 backdrop-blur-md border-b transition-colors duration-300 ${darkMode ? 'bg-slate-900/80 border-slate-700 text-white' : 'bg-white/80 border-slate-200 text-slate-900'
    }`}>
    ...
    <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center z-10">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
        ...
        <button
          ...
          className={`px-8 py-4 rounded-xl font-bold text-lg border-2 transition-all duration-300 w-full sm:w-auto flex items-center justify-center gap-2 ${darkMode ? 'border-slate-700 text-white hover:bg-slate-800' : 'border-slate-200 text-slate-700 hover:bg-slate-50'
            }`}
        >
          <Download size={24} />
...
const SendInterface = ({ setPage, darkMode, isPeerLoaded }) => {
  ...
  return (
    <div className="min-h-screen pt-24 px-4 flex flex-col items-center max-w-4xl mx-auto">
      ...
      <div className={`w-full max-w-2xl rounded-3xl p-8 border-2 border-dashed transition-all duration-300 relative overflow-hidden ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-300'
        }`}>
        ...
        {status === 'init' && (
          <div className="flex flex-col gap-6">
            ...
            <input
              ...
              className={`w-full text-center text-xl font-mono py-4 rounded-xl border-2 focus:outline-none focus:border-cyan-500 uppercase ${darkMode ? 'bg-slate-800 border-slate-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'
                }`}
              value={remoteId}
